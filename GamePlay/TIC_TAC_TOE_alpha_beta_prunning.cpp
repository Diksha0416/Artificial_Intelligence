...
int player_symbol;
int comp_symbol;
int alpha_beta_minimaxCount = 0; // Counter to track alpha_beta_minimax calls
...
int alpha_beta_minimax(int depth, int maxDepth, int alpha, int beta) {
    alpha_beta_minimaxCount++;  // Increment the alpha_beta_minimax call counter
    int score = evaluate_board();

    if (score == 100) return score;      // Computer wins
    else if (score == -100) return score; // Player wins
    else if (!moves_left()) return 0;    // Draw

    if (depth >= maxDepth) return rate(board); 

    // Set bestScore based on the depth
    int bestScore = (depth % 2 == 0) ? -1000 : 1000;

    // Find the next available move recursively
    int nextMove = -1;
    for (int i = 1; i <= 9; i++) {
        if (board[i] == 2) {
            nextMove = i; // Store the next available move
            break; // Exit after finding the first available move
        }
    }

    if (nextMove != -1) { // If a valid move is found
        board[nextMove] = (depth % 2 == 0) ? comp_symbol : player_symbol;

        // Recursively call alpha_beta_minimax
        int result = -alpha_beta_minimax(depth + 1, maxDepth, -beta, -alpha);
        board[nextMove] = 2; // Undo the move

        // Update bestScore based on the depth
        if (depth % 2 == 0) {
        ...
        if (alpha >= beta) return bestScore; // Cut-off
    }
